...
# * Modified to avoid file writes by Lighthouse (foss@lighthouseuk.net) 2016 *
# ****************************************************************************
...
class FPDF(object):
    ...
    def get_font_data(self, data_name):
        return self.ramdisk.get_data(cache_key=data_name)

    def save_font_data(self, data_name, data):
        self.ramdisk.store_data(cache_key=data_name, data=data)
    ...
    def set_display_mode(self, zoom, layout='continuous'):
        ...
        if (zoom == 'fullpage' or zoom == 'fullwidth' or zoom == 'real' or zoom == 'default' or not isinstance(zoom,
                                                                                                               basestring)):
            self.zoom_mode = zoom
        else:
    ...
    def add_font(self, family, style='', fname='', uni=False):
        ...
        if fontkey in self.fonts:
            ...
            return
        if (uni):
            # TODO: modify this whole block. If we're clever we can get rid of all of the checking and just load from
            # the ramdisk
            ...
            if FPDF_CACHE_MODE == 0:
                ...
                unifilename = None
            font_dict = self.get_font_data(data_name=unifilename)
            if font_dict is None:
                ...
                if unifilename:
                    self.save_font_data(data_name=unifilename, data=font_dict)
                del ttf
            ...
            if fontkey in self.core_fonts:
                if fontkey not in fpdf_charwidths:
                    ...
                    if (family == 'times' or family == 'helvetica'):
                        name += style.lower()
                    self.get_font_data(data_name='{}.font'.format(name))
                    if fontkey not in fpdf_charwidths:
                        self.error('Could not include font metric file for' + fontkey)
                i = len(self.fonts) + 1
                self.fonts[fontkey] = {'i': i, 'type': 'core', 'name': self.core_fonts[fontkey], 'up': -100, 'ut': 50,
    ...
    def cell(self, w, h=0, txt='', border=0, ln=0, align='', fill=0, link=''):
        ...
        if (txt != ''):
            ...
            if (self.color_flag):
                s += ' Q'
            if (link):
                self.link(self.x + dx, self.y + .5 * h - .5 * self.font_size, self.get_string_width(txt, True),
                          self.font_size, link)
        if (s):
            self._out(s)
        self.lasth = h
    ...
    def _putfonts(self):
        ...
        for name, info in self.font_files.items():
            if 'type' in info and info['type'] != 'TTF':
                ...
                font = self.get_font_data(data_name='{}{}'.format(self._getfontpath(), name))
        ...
        for idx, k, font in flist:
            ...
            if (type == 'core'):
                ...
                for k in (
                        'Ascent', 'Descent', 'CapHeight', 'Flags', 'FontBBox', 'ItalicAngle', 'StemV', 'MissingWidth'):
                    s += ' /%s %s' % (k, font['desc'][k])
                filename = font['file']
                if (filename):
                    s += ' /FontFile'
    ...
    def _putTTfontwidths(self, font, maxUni):
        if font['unifilename']:
            ...
            cw127fname = None
        font_dict = self.get_font_data(data_name=cw127fname)
        if font_dict is None:
            rangeid = 0
            range_ = {}
            range_interval = {}
        ...
        for cid in range(startcid, cwlen):
            if cid == 128 and cw127fname and not self.get_font_data(data_name=cw127fname):
                font_dict = {'rangeid': rangeid,
                             ...
                             'range': range_}
                self.save_font_data(data_name=cw127fname, data=font_dict)
